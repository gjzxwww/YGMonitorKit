...
typedef struct YGStackFrame {
    const struct YGStackFrame *const previous;
    const uintptr_t return_address;
} YGStackFrame;

//存储 thread 信息的结构体
typedef struct YGThreadInfoStruct {
    double cpuUsage;
    integer_t userTime;
} YGThreadInfoStruct;

static mach_port_t _YGMainThreadId;
@implementation YGCallStack

+ (void)load {
    _YGMainThreadId = mach_thread_self();
}

#pragma mark - Interface
+ (NSString *)callStackWithType:(YGCallStackType)type {
    if (type == YGCallStackTypeAll) {
        ...
        for (int i = 0; i < thread_count; i++) {
            //当前执行的指令
            [reStr appendString:YGStackOfThread(threads[i])];
        }
        //task info
        NSString *memStr = @"";
        ...
        return [reStr copy];
    } else if (type == YGCallStackTypeMain) {
        //主线程
        NSString *reStr = YGStackOfThread((thread_t)_YGMainThreadId);
        assert(vm_deallocate(mach_task_self(), (vm_address_t)_YGMainThreadId, 1 * sizeof(thread_t)) == KERN_SUCCESS);
        NSLog(@"%@",reStr);
        return [reStr copy];
    } else {
        //当前线程
        ...
        [nsthread setName:originName];
        reStr = YGStackOfThread(mach_thread_self());
        NSLog(@"%@",reStr);
        return [reStr copy];
    }
    return @"";
...
//当前栈帧结构
NSString *YGStackOfThread(thread_t thread) {
    //thread info
    YGThreadInfoStruct threadInfoSt = {0};
    ...
    mach_msg_type_number_t state_count = YGThreadStateCountByCPU();
    kern_return_t kr = thread_get_state(thread, YGThreadStateByCPU(), (thread_state_t)&machineContext.__ss, &state_count);
    if (kr != KERN_SUCCESS) {
        return [NSString stringWithFormat:@"Fail get thread: %u", thread];
    }
    //通过指令指针来获取当前指令地址
    const uintptr_t instructionAddress = YGMachInstructionPointerByCPU(&machineContext);
    buffer[i] = instructionAddress;
    ++i;
    
    //
    uintptr_t linkRegisterPointer = YGMachThreadGetLinkRegisterPointerByCPU(&machineContext);
    ...
    YGStackFrame stackFrame = {0};
    //通过栈基址指针获取当前栈帧地址
    const uintptr_t framePointer = YGMachStackBasePointerByCPU(&machineContext);
    if (framePointer == 0 || YGMemCopySafely((void *)framePointer, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
        return @"Fail frame pointer";
    }
    for (; i < 32; i++) {
        buffer[i] = stackFrame.return_address;
        if (buffer[i] == 0 || stackFrame.previous == 0 || YGMemCopySafely(stackFrame.previous, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
            break;
        }
    }
    ...
    YGSymbolicate(buffer, symbolicated, stackLength, 0);
    for (int i = 0; i < stackLength; ++i) {
        [reStr appendFormat:@"%@", YGOutputLog(i, buffer[i], &symbolicated[i])];
    }
    [reStr appendFormat:@"\n"];
    return reStr;
}
...
NSString* YGOutputLog(const int entryNum, const uintptr_t address, const Dl_info* const dlInfo) {
...
kern_return_t YGMemCopySafely(const void *const src, void *const dst, const size_t byteSize) {
    vm_size_t bytesCopied = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)src, (vm_size_t)byteSize, (vm_address_t)dst, &bytesCopied);
}

#pragma mark - Symbolicate
void YGSymbolicate(const uintptr_t* const stackBuffer, Dl_info* const symbolsBuffer, const int stackLength, const int skippedEntries) {
    int i = 0;
    if (!skippedEntries && i < stackLength) {
        YGDladdr(stackBuffer[i], &symbolsBuffer[i]);
        i++;
    }
    for (; i < stackLength; i++) {
        YGDladdr(YGInstructionAddressByCPU(stackBuffer[i]), &symbolsBuffer[i]);
    }
}
bool YGDladdr(const uintptr_t address, Dl_info* const info) {
    info->dli_fname = NULL;
    info->dli_fbase = NULL;
    info->dli_sname = NULL;
    info->dli_saddr = NULL;
    //根据地址获取是哪个 image
    const uint32_t idx = YGDyldImageIndexFromAddress(address);
    if (idx == UINT_MAX) {
        return false;
    }
    /*
    ...
    const uintptr_t segmentBase = YGSegmentBaseOfImageIndex(idx) + imageVMAddressSlide;
    if (segmentBase == 0) {
        return false;
    }
    //
    ...
    uintptr_t cmdPointer = YGCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPointer == 0) {
        return false;
    }
...
uint32_t YGDyldImageIndexFromAddress(const uintptr_t address) {
...
uintptr_t YGCmdFirstPointerFromMachHeader(const struct mach_header* const machHeader) {
...
uintptr_t YGSegmentBaseOfImageIndex(const uint32_t idx) {
    const struct mach_header* machHeader = _dyld_get_image_header(idx);
    //查找 segment command 返回 image 的地址
    uintptr_t cmdPtr = YGCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPtr == 0) {
        return 0;
    }
 ...
 */
uintptr_t YGMachStackBasePointerByCPU(mcontext_t const machineContext) {
...
uintptr_t YGMachInstructionPointerByCPU(mcontext_t const machineContext) {
...
uintptr_t YGInstructionAddressByCPU(const uintptr_t address) {
#if defined(__arm64__)
...
mach_msg_type_number_t YGThreadStateCountByCPU() {
#if defined(__arm64__)
...
*/
thread_state_flavor_t YGThreadStateByCPU() {
#if defined(__arm64__)
...
uintptr_t YGMachThreadGetLinkRegisterPointerByCPU(mcontext_t const machineContext) {
#if defined(__i386__)
    return 0;
#elif defined(__x86_64__)
    return 0;
